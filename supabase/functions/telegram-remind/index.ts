 ...
 *
 * 2. Итог дня — по желанию, около 21:00 ПО МЕСТУ ЧЕЛОВЕКА, с разбивкой по
 *    станциям: не «пять часов», а куда эти пять часов ушли. Расписание одно на
 *    всех, а вечер у каждого свой, поэтому час считает база по сохранённому
 *    часовому поясу (tt_daily_summaries, миграция 008; разбивка —
 *    tt_today_breakdown_for, миграция 009).
 *
...
Deno.serve(async (req) => {
  ...
  for (const row of evening) {
    /**
     * Не просто «столько-то за день», а КУДА оно ушло: одна цифра ничего не
     * добавляет к тому, что человек и так видит в приложении (созвон №7).
     * Разбивка идёт по станциям, от большей к меньшей; если её вдруг не
     * отдали — сообщение всё равно уходит, с одним итогом.
     */
    const { data: parts, error: partsError } = await admin.rpc('tt_today_breakdown_for', {
      target_user: row.user_id,
    });
    if (partsError) console.error('tt_today_breakdown_for', partsError.message);
    const lines = ((parts ?? []) as { name: string; seconds: number }[])
      .map((p) => `• ${p.name} — ${human(p.seconds)}`)
      .join('\n');

    await sendMessage(
      row.chat_id,
      `🌙 Итог дня: <b>${human(row.seconds)}</b>.${lines ? `\n\n${lines}` : ''}\n\nЗавтра продолжим — карта ждёт.`,
    );
    await admin
      .from('tt_telegram_links')
